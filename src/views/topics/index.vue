  ...
  <div class="app-container">
    ...
    >
      ...
      >
        <template slot-scope="{ row }">
          <el-button-group>
            <el-button type="success" @click="onToggleSelection(row,1)">查看内容</el-button>
            <el-button type="success" @click="onToggleSelection(row,1)">查看评论</el-button>
...
import { audit, dataList, deleteData, exportOrder } from '@/api/topics'
...
export default {
  name: 'Topic',
  components: { Pagination, ElImageViewer, AddOrUpdate, Swiper, SwiperSlide },
  data() {
    return {
      swiperOption: {
        slidesPerView: 3,
  ...
  methods: {
    ...
    onToggleSelection(row, type) {
      row = Array.isArray(row) ? row : [row]
      const row_id = row.map(v => `${v.id}`)
      const row_title = row.map(v => `${v.title}`)

      const title = type === 1 ? '通过' : '驳回'
      const confirmType = type === 1 ? this.$confirm : this.$prompt
      confirmType(`${title}话题「#${row_id}」<span style="color: #eb7340;">#${row_title}#</span> ？`, `${title}话题`, {
