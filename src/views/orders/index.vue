      ...
      >
        <el-table-column>
          ...
          <el-table-column min-width="240" header-align="center">
            <template slot-scope="{ row: { goods },row }">
              <div v-if="goods" class="info-wrapper">
                <el-image
                  class="image-item"
                  :src="Array.isArray(goods.images) && goods.images[0] && goods.platform_info.oss_url + goods.images[0]"
                  :preview-src-list="[Array.isArray(goods.images) && goods.images[0] && goods.platform_info.oss_url + goods.images[0]]"
                >
          ...
          <el-table-column min-width="100" header-align="center">
            <template slot-scope="{ row: { goods: { platform_info } } }">
              <div>
                <el-image class="icon-logo" fit="cover" :src="platform_info.icon && domin + platform_info.icon" />
                {{ platform_info.name }}
              </div>
            </template>
          </el-table-column>
    ...
    getList(page = this.pages.current, loading = true) {
      ...
      dataList({ page, ...this.search, limit: this.pages.limit })
        .then((response) => {
          ...
          this.statusCount.set(4, response.data.intervene_count || 0)
        })
        .catch(() => { })
        .finally(() => {
          this.loading = false
