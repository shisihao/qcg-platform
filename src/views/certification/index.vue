<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" border highlight-current-row :data="list">
      ...
      <el-table-column label="认证信息" width="200" header-align="center">
      ...
      <el-table-column label="认证结果" min-width="200" header-align="center">
        <template slot-scope="{ row: { result } }">
          ...
          <div v-else-if="result && result.pictureUrl">
            <el-image
              ...
              :preview-src-list="[result.pictureUrl && domainUrl + result.pictureUrl]"
            >
      ...
      <el-table-column label="状态" width="80" align="center">
        <template slot-scope="{ row: { status, admin } }">
          <div v-if="[0, 2].includes(status)">
            <el-tag slot="reference" :type="[0, 2].includes(status) && 2 | paraphrase(statusOptions, 'value', 'type')">
              {{ [0, 2].includes(status) && 2 | paraphrase(statusOptions) }}
            </el-tag>
          </div>
          <div v-else>
            <el-tag :type="status | paraphrase(statusOptions, 'value', 'type')">
      ...
      <el-table-column label="是否删除" width="80" align="center">
        <template slot-scope="{ row }">
          <span :style="`color: ${row.is_delete ? '#ff4949' : ''}`">{{ row.is_delete | paraphrase(whetherOptions)
          }}</span>
        </template>
      </el-table-column>
...
import { dataList, statusPass, exportOrder } from '@/api/certification'
...
export default {
  ...
  data() {
    return {
      ...
      list: [],
      wait_count: 0,
      domainUrl: 'https://cn-shanghai-aliyun-cloudauth-1941952835563176.oss-cn-shanghai.aliyuncs.com/',
      search: {
        user_name: '',
        number: '',
        status: '',
