  ...
  <el-dialog title="查看邀请" :visible.sync="visible" @closed="onClose()">
    ...
    <p>
      <el-row>
        <el-col :span="6" style="line-height: 24px;">
          团队总人数：{{ teamInfo.totalCount }}
        </el-col>
        <el-col :span="6" style="line-height: 24px;">
          已实名人数：{{ teamInfo.certCount }}
        </el-col>
        <el-col :span="6" style="line-height: 24px;">
          未实名人数：{{ teamInfo.noCertCount }}
        </el-col>
      </el-row>
    </p>
    <el-divider />
    <el-tree v-if="openPanel" lazy :load="loadNode" :props="defaultProps">
      <span slot-scope="{ node }">
        ...
        {{ node.data.phone }}
        <el-divider direction="vertical" />
        {{ node.data.cert_status ===1?'已实名':'未实名' }}
        <el-divider v-if="node.data.cert_status ===1" direction="vertical" />
        {{ node.data.cert_status ===1? node.data.cert_time:'' }}
      </span>
    </el-tree>
  </el-dialog>
</template>
